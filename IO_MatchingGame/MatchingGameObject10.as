package {
	...
	public class MatchingGameObject10 extends MovieClip {
		// game constants
		private var boardWidth:uint = 6;
		private var boardHeight:uint = 6;
		...
		private static const pointsForMatch:int = 20;
		private static const pointsForMiss:int = 0;
		...
		private var currentPlayerField:TextField;
		...
		public function MatchingGameObject10():void {
			...
			//if ()
			boardWidth = MovieClip(root).widthHeight;
			boardHeight = MovieClip(root).widthHeight;
		
			player1 = MovieClip(root).player1; //when currentPlayer < 1
			player2 = MovieClip(root).player2; //when currentPlayer > 0
			player1Score = MovieClip(root).player1Score; //when currentPlayer < 1
			player2Score = MovieClip(root).player2Score; //when currentPlayer > 0
			...
			currentPlayerField = new TextField();
			addChild(gameScoreField);
			currentPlayerField.y = 10;
			addChild(currentPlayerField);
			//gameScore = 0;
			//player1Score = 0;
			//player2Score = 0;
			showGameScore();
		...
		public function clickCard(event:MouseEvent) {
			...
			if (firstCard == null) { // first card in a pair
				...
				if (firstCard.cardface == secondCard.cardface) {
					...
					if (currentPlayer < 1) player1Score += pointsForMatch;
					else player2Score += pointsForMatch;
					showGameScore();
					...
					if (cardsLeft == 0) {
						...
						MovieClip(root).player1Score = player1Score;
						MovieClip(root).player2Score = player2Score;
		...
		public function showGameScore() {
			...
			gameScoreField.text = player1+": "+String(player1Score)+" - "+player2+": "+String(player2Score);
			if(currentPlayer < 1) currentPlayerField.text = player1+" plays now";
			else currentPlayerField.text = player2+" plays now";
		}
